...
public class BeneathEventHandler {
	...
	public void darkness(LivingUpdateEvent event){
		...
		if(event.getEntityLiving() instanceof EntityPlayer && event.getEntityLiving().world.provider.getDimension() == Beneath.dim){
			...
			if(player.getBrightness() <= 0.05F && player.ticksExisted % (Beneath.darkTimer * 20) == 0)
	...
	public void boostStats(EntityJoinWorldEvent event){
		if(event.getEntity() instanceof EntityMob && event.getWorld().provider.getDimension() == Beneath.dim){
			...
			AttributeModifier dmg = new AttributeModifier(uuid1, "The Beneath Attack Damage Boost", damage.getBaseValue() * Beneath.damageMultiplier - damage.getBaseValue(), 0);
			AttributeModifier hp = new AttributeModifier(uuid2, "The Beneath Health Boost", health.getBaseValue() * Beneath.healthMultiplier - health.getBaseValue(), 0);
